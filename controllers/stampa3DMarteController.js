const Stampa3DMarte = require('../models/Stampa3DMarte');
...
exports.registraStampante = async (req, res) => {
  try {
    const stampante = new Stampa3DMarte({
      ...req.body,
...
exports.getStampanti = async (req, res) => {
  try {
    const stampanti = await Stampa3DMarte.find({ proprietarioId: getUserId(req) });
...
exports.getStampanteDetails = async (req, res) => {
  try {
    const stampante = await Stampa3DMarte.findById(req.params.id);
    if (!stampante) {
...
exports.updateStampante = async (req, res) => {
  try {
    const stampante = await Stampa3DMarte.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteStampante = async (req, res) => {
  try {
    await Stampa3DMarte.findByIdAndDelete(req.params.id);
...
exports.creaProgetto = async (req, res) => {
  try {
    ...
    if (stampante.materiali.regolite.disponibile < (materiali.regolite || 0) ||
        ...
        stampante.materiali.metalli.disponibile < (materiali.metalli || 0) ||
        stampante.materiali.biomateriali.disponibile < (materiali.biomateriali || 0)) {
...
exports.completaStampa = async (req, res) => {
  try {
    ...
    // Calcola ricavo
    const ricavo = (progetto.materialeUsato?.regolite || 0) * 8 +
                   (progetto.materialeUsato?.polimeri || 0) * 40 +
                   (progetto.materialeUsato?.metalli || 0) * 80 +
                   (progetto.materialeUsato?.biomateriali || 0) * 120;
    
    stampante.statistiche.ricaviTotali += ricavo;
    await stampante.save();
