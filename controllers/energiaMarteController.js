const EnergiaMarte = require('../models/EnergiaMarte');
...
exports.registraEnergia = async (req, res) => {
  try {
    const energia = new EnergiaMarte({
      ...req.body,
...
exports.getEnergia = async (req, res) => {
  try {
    const energia = await EnergiaMarte.find({ proprietarioId: getUserId(req) });
...
exports.getEnergiaDetails = async (req, res) => {
  try {
    const energia = await EnergiaMarte.findById(req.params.id);
    if (!energia) {
...
exports.updateEnergia = async (req, res) => {
  try {
    const energia = await EnergiaMarte.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteEnergia = async (req, res) => {
  try {
    await EnergiaMarte.findByIdAndDelete(req.params.id);
