const FabbricaMarte = require('../models/FabbricaMarte');
...
exports.registraFabbrica = async (req, res) => {
  try {
    const fabbrica = new FabbricaMarte({
      ...req.body,
...
exports.getFabbriche = async (req, res) => {
  try {
    const fabbriche = await FabbricaMarte.find({ proprietarioId: getUserId(req) });
...
exports.getFabbricaDetails = async (req, res) => {
  try {
    const fabbrica = await FabbricaMarte.findById(req.params.id);
    if (!fabbrica) {
...
exports.updateFabbrica = async (req, res) => {
  try {
    const fabbrica = await FabbricaMarte.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteFabbrica = async (req, res) => {
  try {
    await FabbricaMarte.findByIdAndDelete(req.params.id);
...
exports.creaOrdine = async (req, res) => {
  try {
    const { prodottoId, quantita, priorita } = req.body;
    const fabbrica = await FabbricaMarte.findById(req.params.id);
    if (!fabbrica) {
      return res.status(404).json({ error: 'Fabbrica non trovata' });
    }
    ...
    if (fabbrica.magazzino.materiePrime.regolite < (materiali.regolite || 0) * quantita ||
        ...
        fabbrica.magazzino.materiePrime.minerali < (materiali.minerali || 0) * quantita) {
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        lineeProduzione: fabbrica.lineeProduzione
      }
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
