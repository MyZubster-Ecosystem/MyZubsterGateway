const CittaLunare = require('../models/CittaLunare');
...
exports.getCitta = async (req, res) => {
  try {
    const citta = await CittaLunare.find({ proprietarioId: getUserId(req) });
...
exports.getCittaDetails = async (req, res) => {
  try {
    const citta = await CittaLunare.findById(req.params.id);
    if (!citta) {
...
exports.updateCitta = async (req, res) => {
  try {
    const citta = await CittaLunare.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteCitta = async (req, res) => {
  try {
    await CittaLunare.findByIdAndDelete(req.params.id);
...
exports.getStats = async (req, res) => {
  try {
    ...
    const qualita = Math.min(100, 
      (citta.statistiche.verdePubblico / citta.layout.superficieTotale * 100) * 0.3 +
