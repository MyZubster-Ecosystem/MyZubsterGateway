const BaseMarte = require('../models/BaseMarte');
...
exports.getBasi = async (req, res) => {
  try {
    const basi = await BaseMarte.find({ proprietarioId: getUserId(req) });
...
exports.getBaseDetails = async (req, res) => {
  try {
    const base = await BaseMarte.findById(req.params.id);
    if (!base) {
      return res.status(404).json({ error: 'Base Marte non trovata' });
    }
...
exports.updateBase = async (req, res) => {
  try {
    const base = await BaseMarte.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteBase = async (req, res) => {
  try {
    await BaseMarte.findByIdAndDelete(req.params.id);
    res.json({ success: true, message: 'Base Marte eliminata' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
